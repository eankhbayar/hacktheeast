...
/**
 * Stub question bank, keyed by topic. Will be replaced by the
 * Orchestrator -> Generator agent pipeline.
 */
...
function pickFromBank(profile: ChildProfile): { q: string; options: string[]; answer: string; topic: string } {
  const topics = profile.learningFocus.length > 0 ? profile.learningFocus : ['general'];
...
export async function generateQuestion(
  ...
  const picked = pickFromBank(profile || { learningFocus: [], interests: [] } as unknown as ChildProfile);
